(** Abstraction Function: The game [game] contains information about the chess
    play (whose turn it is, the current board, castling preconditions, whether
    the welcome screen is shown, and previous/future turns). [game.title]
    represents the last move taken. Representation invariant: [game.past] should
    only contain previous moves and [game.future] should only contain future
    moves that are wiped out when a new move from a previous board is made. The
    previous and future games alternate in black/white turn. *)
...
let update_castle piece source destination game =
  let king_side = source.x < destination.x in
  let rook_pos = { x = (if king_side then 7 else 0); y = source.y } in
  let rook_end =
    { x = (if king_side then -1 else 1) + destination.x; y = destination.y }
  in
  Square_matrix.set rook_end (get_piece rook_pos game) game.board;
  Square_matrix.set rook_pos empty game.board;
  {
    game with
    title = (player_string piece.player ^ if king_side then " 0-0" else " 0-0-0");
  }

let update_game source destination game =
  let piece = get_piece source game in
  match piece.kind with
  | King when abs (source.x - destination.x) = 2 ->
      update_castle piece source destination game
  ...
  | Pawn
    when destination.x != source.x && (get_piece destination game).kind = Empty
    ->
...
let update_move source destination game =
  let updated_game = update_game source destination game in
...
let update_turn game = { game with turn = other game.turn }
...
let allow_pawn_forward source dst game =
  (get_piece dst game).player = None
  &&
  let len = dst.y - source.y in
  abs len = 1 || (len = 2 && dst.y = 3) || (len = -2 && dst.y = 4)

(** Pawn capture with en passant. *)
let allow_pawn_capture source dst game =
  (* Take down opponent *)
  ...
  let start =
    Position.move alongside { x = 0; y = (if source.y < dst.y then 2 else -2) }
  in
  String.ends_with
    ~suffix:
...
let allow_piece_move source dst game =
  ...
  | Pawn when source.x = dst.x -> allow_pawn_forward source dst game
  | Pawn -> allow_pawn_capture source dst game
  | _ -> true
...
let player_move_click pos game =
  ...
  else if is_valid pos && List.mem pos (get_valid_moves game.selected game) then (
    let next = move game.selected pos game_unselected in
    Printf.printf "%s\n%!" next.title;
    next)
  else game_unselected

let player_move_key key game =
...
let player_move (inputs : unit -> Display.user_input) game =
  ...
  match inputs () with
  | Click pos -> player_move_click pos game
  | Key key -> player_move_key key game
